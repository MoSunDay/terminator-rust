...
/// Prepend the primary mono font to and append the CJK fallback font to
/// both family fallback chains. Idempotent: a second call must not
/// duplicate either entry.
...
pub fn install(ctx: &egui::Context) -> bool {
    ...
    ctx.set_fonts(defs);
    log::info!("terminal fonts installed: {MONO_FONT_NAME} + {CJK_FONT_NAME}");
    true
}
...
mod tests {
    ...
    fn push_families_brackets_chain_and_idempotent() {
        ...
        for name in [MONO_FONT_NAME, CJK_FONT_NAME] {
