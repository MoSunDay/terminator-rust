...
- releases: push an annotated `v*` tag -> .github/workflows/release.yml
  ...
  sidecar; macOS needs `brew install gnu-tar`, the script picks up
  gtar) and publishes the GitHub Release whose notes = the annotated
  tag body + a checksums table; publish is tag-only (workflow_dispatch
  rebuilds artifacts). Tag body is written with `git tag -a vX -F`.
- logo: assets/logo generated by scripts/bin/gen-logo.py (deterministic,
  Pillow only; `--check` byte-drift gate in the ci.yml build-test job
  compares ALL 13 files - regenerate with `python3 scripts/bin/gen-logo.py`);
  crates/app/src/app_icon.rs embeds a 256px raster -> with_icon on the root
  viewport AND every secondary (windows.rs builder_for); deploy packs a
  hicolor icon tree + autostart desktop Icon=. ONLY THE MACOS ART carries
  the Apple icon-template margin (body 824/1024 = a transparent ring;
  mac_margin/mac_raster): the NEW assets/logo/icon-256-mac.png (ICON_PNG is
  a cfg pair - mac raster on target_os = "macos", full-bleed icon-256.png
  elsewhere) AND every .icns entry; icon-*.png/.ico stay FULL-BLEED because
  Linux/Windows scale the icon into their own slot (a margin there renders
  visibly smaller). WHY - do NOT "fix" the padding away: eframe's
  AppTitleIconSetter publishes our RGBA VERBATIM through
  `NSApp setApplicationIconImage:` and winit's set_window_icon is a NO-OP on
  macOS, so AppKit scales the WHOLE canvas into the icon slot and full-bleed
  art renders ~24% LARGER than every other Dock icon (1024/824). Guards:
  gen-logo.py verify_mac (transparent margin ring, inked body edge, SAMPLES
  hues remapped into the body) on icon-256-mac + all six icns entries, and a
  full-bleed regression assert (edge alpha > 128; measures 239/195/254 at
  48/256/1024 from LANCZOS border clipping - a mac template reads 0).
  app_icon tests DERIVE geometry (ink_box alpha bbox + design_px mapping the
  1024u design grid into it) instead of hardcoded 256-grid coords, so ONE
  assert body is right on both platforms, plus per-platform ink_box guards
  ((25,25,206) macOS / (0,0,256) elsewhere). RADIUS/S == 0.225 already
  equals Apple's body corner ratio -> margin only, no shape change; the
  .icns shrank 205356 -> 165588 bytes, every other asset byte-identical
  (pack-release.sh darwin branch installs only the .icns - unchanged).
...
- Xvfb smoke ops: launch with `setsid nohup ... </dev/null &` to survive
  ...
  `xprop -id W _NET_WM_ICON` prints the property NAME with an EMPTY
...
- GPU surface-size guard (2026-09-22 crash): wgpu ABORTS the process
  ...
  scale already crosses it) - eframe configures the surface inside the
  winit Resized handler BEFORE any app frame, so an after-the-fact clamp
  is always too late. app/src/render/surface_guard.rs: WM-level
  max-inner-size constraint (ViewportBuilder with_max_inner_size at
  launch + ViewportCommand::MaxInnerSize refined each frame to the real
  adapter limit from i.max_texture_side; oversized windows also get
  InnerSize clamped back). apply() is called in the root pass (main.rs)
...
- control socket: $XDG_RUNTIME_DIR/terminator-rust/ipc.sock (fallback
  ...
  control; the ~/.terminator-rust fallback dir is traversable on common distros);
...
- IME (pane input method): winit X11 XIM is COMPLETE (XFilterEvent
  ...
  vertical, clipped to the pane) and underlines with egui's
  visuals.ime_composition.active_underline_stroke (2px square stroke,
  rgb(192,222,255) dark default) - the SAME stroke every TextEdit in
  the app uses. winit X11 consumes ONLY IMEOutput.rect.min as the raw
  XIM spot (size ignored): ime.rs anchor_rect drops it to the caret
  cell BOTTOM-left (xterm/GTK baseline convention; candidates float
  below the composing line instead of a row high); Wayland/macos
  (WAYLAND_DISPLAY set) keep the true cell. e2e realities: the
  composition stroke ANTIALIASES (exact-color match finds zero pixels
  - blue-hue detector b-g>=18 r-g<=5 b>=110 vs baseline scrot), and
  libpinyin starts in CN mode (all latin keys compose: probe shell
  liveness via ctl, ASCII needs the bare Shift_L EN toggle).
- chrome row height: nominal constants are NOT the rendered height -
  ...
  arithmetic.
- chrome scales with the terminal font (2026-09-22): ALL chrome metrics
...
- font stack: ui/fonts.rs installs TWO embedded OFL fonts. PRIMARY
  ...
  color glyphs; fontTools needs lxml to subset SVG). FOURTH font:
  assets/fonts/NotoEmoji-subset.ttf (364KB, 605 glyphs, OFL) pushed
  LAST (after terminator-symbols) - egui's built-in NotoEmoji-Regular
  LACKS U+1F916 (opencode's 🤖 avatar) + U+FE0F, so 🤖 fell through
  the whole chain to a literal '?' inside its 2-cell span (right half
  = a hole: the 乱码 AND the 断层 were one bug). Subset = the exact
  cmap COMPLEMENT of egui's emoji face (same Noto outline style),
  source google/fonts ofl/notoemoji NotoEmoji[wght].ttf via
  cdn.jsdelivr.net (github/raw blocked), varLib.instancer wght=400
  FIRST (pyftsubset has no --instancer), then the usual
  --no-layout-closure --no-hinting. e2e-cjk gates its cmap
  (🤖🤔🧠🫡+FE0F) and a live `汉字测试🤖` round-trip. opencode welcome's
  👋 row losing its 2-space indent is UPSTREAM (ratatui writes from
  col2; pty-sniffed), not a terminal bug. ghostty advances
...
- window shell: BORDERLESS again (re-reverted the same day 2026-09-20,
  ...
  (clear_color = TRANSPARENT); Settings.opacity (0.1..=1.0, default
