...
#                  dist/ (staging dir incl. the hicolor icon tree +
...
#                  for the desktop user (Exec=/Icon= via the `current`
#                  symlink).
...
stage_build() {
    ...
    # hicolor icon tree (autostart desktop entry's Icon= points at the
    # 256x256 one); assets/logo is generated by scripts/bin/gen-logo.py.
...
icon=$root/current/share/icons/hicolor/256x256/apps/terminator-rust.png
grep -Fx "Icon=$icon" "$d/terminator-rust.desktop"
test -f "$icon"
